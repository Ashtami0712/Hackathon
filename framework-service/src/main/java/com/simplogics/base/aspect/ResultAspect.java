...
public class ResultAspect {
	...
	public Object onApiResponse(final ProceedingJoinPoint pjp) {
		...
		FrameworkBaseResponse result;
		HttpStatus httpStatus;
		try {
			return pjp.proceed();
		} catch (Throwable e) {
            switch (e) {
                case FrameworkException exception -> {
                    httpStatus = exception.getStatus();
                    result = new FrameworkBaseResponse(false, null, Translator.translateToLocale(exception.getMessage()), true,
                            Collections.singletonList(exception.getMessage()));
                }
                case BadCredentialsException exception -> {
                    httpStatus = HttpStatus.UNAUTHORIZED;
                    result = new FrameworkBaseResponse(false, null, Translator.translateToLocale("incorrect.credentials"), true,
                            Collections.singletonList(exception.getMessage()));
                }
                ...
                default -> {
                    ...
                    result = new FrameworkBaseResponse(false, null, Translator.translateToLocale(apiResult.error_message()), true,
