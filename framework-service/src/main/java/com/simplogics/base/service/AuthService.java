...
public class AuthService implements IAuthService {
	...
	public LoginResponse login(LoginRequest loginDto, User user) throws Exception {
		if(!user.getIsActive())
			throw new FrameworkException("user.not.active", HttpStatus.BAD_REQUEST);
	...
	@Override
	public void forgetPassword(User user) throws FrameworkException {
		if(!user.getIsActive())
			throw new FrameworkException("user.not.active", HttpStatus.BAD_REQUEST);
	...
	@Override
	public String validateTokenAndGetSubject(String token) throws FrameworkException {
		if(passwordResetTokenUtil.isTokenExpired(token)){
			throw new FrameworkException("reset.password.token.expired", HttpStatus.BAD_REQUEST);
		}
		return passwordResetTokenUtil.getSubject(token);
	}

	private void generateAndSendEmail(User user, String resetUrl) throws FrameworkException {
